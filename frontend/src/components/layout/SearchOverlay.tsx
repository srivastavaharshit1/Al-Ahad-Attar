...
export const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <button 
        onClick={onClose}
        className="absolute top-8 right-8 text-on-surface-variant hover:text-primary transition-colors p-2 rounded-full hover:bg-surface-container"
      >
      ...
      <div className="max-w-container-max mx-auto w-full flex-grow flex flex-col gap-section-gap pb-24">
        <div className="flex flex-col items-center text-center space-y-8 w-full max-w-3xl mx-auto">
          <h1 className="font-display-lg text-display-lg text-primary tracking-tight">Discover Fragrances</h1>
          <form 
          ...
          >
            <span className="material-symbols-outlined absolute left-6 top-1/2 transform -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors" style={{ fontSize: '28px' }}>search</span>
            <input 
              ref={inputRef}
              className="w-full bg-transparent border-b border-outline-variant py-6 pl-16 pr-6 font-headline-md text-headline-md text-on-surface placeholder:text-outline-variant/60 focus:border-primary focus:ring-0 transition-colors bg-surface-container-lowest/50 caret-primary focus:outline-none" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter mt-12 w-full">
            ...
            <div className="space-y-6">
              ...
              <ul className="space-y-4">
                {['Oud Wood', 'Saffron Blends', 'White Musk', 'Rose Taifi'].map(term => (
                  <li key={term}>
                    <Link to="/collection" onClick={onClose} className="font-body-lg text-body-lg text-on-surface hover:text-primary transition-colors flex items-center group">
                      <span className="material-symbols-outlined opacity-0 group-hover:opacity-100 -ml-6 mr-2 transition-all text-primary">arrow_right</span>
                      {term}
                    </Link>
                  </li>
                ))}
            ...
            <div className="space-y-6 mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {['Sandalwood', 'Amber', 'Gift Sets'].map(term => (
                  <Link key={term} to="/collection" onClick={onClose} className="px-4 py-2 bg-surface-container border border-outline-variant/30 rounded-full font-label-md text-label-md text-on-surface-variant hover:border-primary hover:text-primary transition-colors">
                    {term}
                  </Link>
                ))}
              </div>
          ...
          <div className="md:col-span-8 md:pl-12 border-t md:border-t-0 md:border-l border-outline-variant/30 pt-12 md:pt-0">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Product 1 */}
              <Link to="/product/1" onClick={onClose} className="group flex flex-col gap-4 p-4 rounded-xl hover:bg-surface-container-lowest transition-colors border border-transparent hover:border-outline-variant/30 shadow-none hover:shadow-sm">
                ...
                <div>
                  <h4 className="font-headline-md text-headline-md text-on-surface group-hover:text-primary transition-colors line-clamp-1">Royal Amber</h4>
                  <p className="font-body-md text-body-md text-on-surface-variant mt-1">Rich, woody, sweet</p>
                </div>
              </Link>
              {/* Product 2 */}
              <Link to="/product/2" onClick={onClose} className="group flex flex-col gap-4 p-4 rounded-xl hover:bg-surface-container-lowest transition-colors border border-transparent hover:border-outline-variant/30 shadow-none hover:shadow-sm">
                <div className="aspect-square bg-surface-container rounded-lg overflow-hidden relative">
                ...
                <div>
                  <h4 className="font-headline-md text-headline-md text-on-surface group-hover:text-primary transition-colors line-clamp-1">White Musk Pure</h4>
                  <p className="font-body-md text-body-md text-on-surface-variant mt-1">Clean, powdery, soft</p>
                </div>
              </Link>
              {/* Product 3 */}
              <Link to="/product/3" onClick={onClose} className="group flex flex-col gap-4 p-4 rounded-xl hover:bg-surface-container-lowest transition-colors border border-transparent hover:border-outline-variant/30 shadow-none hover:shadow-sm hidden lg:flex">
                <div className="aspect-square bg-surface-container rounded-lg overflow-hidden relative">
                ...
                <div>
                  <h4 className="font-headline-md text-headline-md text-on-surface group-hover:text-primary transition-colors line-clamp-1">Oud Al Layl</h4>
